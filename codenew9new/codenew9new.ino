...
SoftwareSerial mySerialLL(RXD2_PIN, TXD2_PIN);
Adafruit_Thermal printer(&mySerialLL);
XT_Wav_Class ForceWithYou(rupees);     // create an object of type XT_Wav_Class that is used by  // the dac audio class (below), passing wav data as parameter                                   XT_Wav_Class ForceWithYou(rupees);
  ...
  if (n<0) {
    ...
    if (n>19) {
      ...
      switch (tens) {
        ...
        case 3: voice.say(spTHIR_); voice.say(spT);
        break;
        case 4: voice.say(spFOUR); voice.say(spT);  
        break;
        case 5: voice.say(spFIF_);  voice.say(spT); break;
        case 6: voice.say(spSIX);  voice.say(spT); 
        break;
        case 7: voice.say(spSEVEN);  voice.say(spT); 
        break;
        case 8: voice.say(spEIGHT);  voice.say(spT); 
        break;
        case 9: voice.say(spNINE);  voice.say(spT); 
        break;
      }
      n %= 10;
    }
    switch(n) {
      ...
      break;
      case 10: voice.say(spTEN); 
      break;
      case 11: voice.say(spELEVEN); 
      break;
      case 12: voice.say(spTWELVE); 
      break;
      case 13: voice.say(spTHIR_); voice.say(sp_TEEN); 
      break;
      case 14: voice.say(spFOUR); voice.say(sp_TEEN);
      break;
      case 15: voice.say(spFIF_); voice.say(sp_TEEN); 
      break;
      case 16: voice.say(spSIX); voice.say(sp_TEEN); 
      break;
      case 17: voice.say(spSEVEN); voice.say(sp_TEEN); 
      break;
      case 18: voice.say(spEIGHT); voice.say(sp_TEEN); 
      break;
      case 19: voice.say(spNINE); voice.say(sp_TEEN); 
      break;
    }
  }
}
...
<html lang=\"en\">\
  ...
  <body>\
          ...
          <div>\
          <br><button id=\"speak_button\">Send</button>\
          </div>\
...
void printerr()
{
  Serial.println("printer func executing");
  printer.boldOn();
  printer.justify('C');
  printer.setSize('M'); 
  printer.println("Whiz IT Services");
  printer.boldOff();
  printer.setSize('S'); 
  printer.println("IT Consulting | Software Development | IoT Solutions Company in Pune-India");
  printer.println("Phone : 07020099372 ");
  printLocalTime();
  printer.setSize('S'); 
  printer.println("******************************");
  printer.setSize('M'); 
   printer.println("");
  printer.print("You Have Received " + server.arg("Number"));
  String serverData = String(server.arg("Number"));
  int finalServer = serverData.toInt();
  printer.print("/- Rs");
  printer.println("");
